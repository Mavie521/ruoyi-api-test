"""
数据提取工具 —— JSON 提取 + SQL 提取，写入全局变量池

快速索引:
  do_extract_json(case, res, global_vars)  — 从响应 JSON 按 JSONPath 提取
  do_extract_sql(case, global_vars)        — 从数据库查询结果提取

触发条件:
  jsonExData 列非空 → do_extract_json()
  sqlExData  列非空 → do_extract_sql()

字段格式（均为 JSON 字典）:
  {"变量名": "JSONPath表达式"}     — jsonExData
  {"变量名": "查询SQL语句"}       — sqlExData
"""
import json
import allure
import jsonpath
import requests
import mysql.connector
from utils.logger import logger
from utils.db_utils import DbClient


# ╔══════════════════════════════════════════════════════════════════════════╗
# ║                        内部工具                                         ║
# ╚══════════════════════════════════════════════════════════════════════════╝

def _parse_ex_dict(raw, field_name: str) -> dict:
    """将 Excel 列的原始值解析为 dict，统一校验逻辑

    两个提取函数的 JSON 解析 + dict 校验完全一致，抽出来消除重复。
    返回解析后的 dict；格式错误时 raise ValueError。
    """
    if isinstance(raw, str):
        try:
            ex_dict = json.loads(raw)
        except json.JSONDecodeError as e:
            logger.error(f"{field_name} JSON 解析失败 | {raw} | {e}")
            raise ValueError(
                f"\n  [{field_name} 格式错误] 非合法 JSON 字典"
                f"\n  内容: {raw}"
                f"\n  错误: {e}"
            ) from e
    else:
        ex_dict = raw

    if not isinstance(ex_dict, dict):
        logger.error(f"{field_name} 须为 JSON 字典 | 实际类型: {type(ex_dict).__name__}")
        raise ValueError(
            f"\n  [{field_name} 格式错误] 须为 JSON 字典（key-value 格式）"
            f"\n  内容: {raw}"
        )

    return ex_dict


# ╔══════════════════════════════════════════════════════════════════════════╗
# ║                    JSON 数据提取（从接口响应）                           ║
# ╚══════════════════════════════════════════════════════════════════════════╝

def do_extract_json(case: dict, res, global_vars: dict):
    """从接口响应 JSON 中按 JSONPath 提取变量 → 全局变量池

    异常分层:
      - JSON 解析异常   → ValueError（阻断）
      - JSONPath 无匹配 → logger.warning（跳过，不影响主流程）
    """
    raw = case.get("jsonExData")
    if not raw:
        return

    with allure.step("JSON 数据提取"):
        ex_dict = _parse_ex_dict(raw, "jsonExData")

        body = res.json() if isinstance(res, requests.Response) else res
        for var_name, jp_expr in ex_dict.items():
            vals = jsonpath.jsonpath(body, jp_expr)
            if vals:
                global_vars[var_name] = vals[0]
                logger.info(f"JSON 提取 | {var_name} = {vals[0]} (from {jp_expr})")
            else:
                logger.warning(f"JSON 提取无匹配 | {var_name} | {jp_expr}")


# ╔══════════════════════════════════════════════════════════════════════════╗
# ║                    SQL 数据提取（从数据库查询）                         ║
# ╚══════════════════════════════════════════════════════════════════════════╝

def do_extract_sql(case: dict, global_vars: dict):
    """从数据库查询结果中提取变量 → 全局变量池

    异常分层:
      - JSON 解析异常 → ValueError（阻断）
      - SQL 执行异常  → logger.error（跳过，不影响主流程）
      - 查询无结果    → logger.warning
    """
    raw = case.get("sqlExData")
    if not raw:
        return

    with allure.step("SQL 数据提取"):
        ex_dict = _parse_ex_dict(raw, "sqlExData")

        db = DbClient()
        try:
            for var_name, sql in ex_dict.items():
                try:
                    row = db.query_one(sql)
                    if row:
                        val = list(row.values())[0]
                        global_vars[var_name] = val
                        logger.info(f"SQL 提取 | {var_name} = {val}")
                    else:
                        logger.warning(f"SQL 提取无结果 | {var_name} | {sql}")
                except mysql.connector.Error as e:
                    logger.error(f"SQL 提取执行异常 | {var_name} | {sql} | {e}")
                    continue
        finally:
            db.close()
